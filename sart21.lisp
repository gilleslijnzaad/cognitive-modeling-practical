...
(p check-current-goal
  ...
  ?goal>
    buffer        empty
  ?visual>
  - scene-change  T       ; check if there is no scene change
==>
  =retrieval>
...
(p identify-stimulus
  ...
  =retrieval>
    isa         goal
    state       attend
  =visual-location>       ; is there a chunk in the visual location buffer?
  ?visual>
    state       free
==>
  +visual>
...
==>
  +manual>
    isa       punch       ; built-in property of manual buffer
    hand      =hand
    finger    index
  -goal>
  -visual-location>
